...
(function(){
    ...
    const DisMultiplier = (Dis: number):number => {
        let el:any = document.evaluate(
            "//*[@id=\"channel-player\"]/div/div[2]/div[1]/span",
            document,
            null,
        ...
        if (el) {
            let Multiplier:number = parseFloat((el!.textContent || el!.innerHTML).slice(0, -1));
    ...
    const CheckElemete = setInterval(()=>{
        if (TimeOut == 600) { clearInterval(CheckElemete) }
        let Duration = document.querySelector<HTMLParagraphElement>('[data-a-target="player-seekbar-duration"]');
        if(Duration != null){
            let TimeDuration = new Date("1970-01-01T" + (Duration!.textContent || Duration!.innerHTML)).getTime();
            document.querySelector<HTMLParagraphElement>('[data-a-target="player-seekbar-current-time"]')!.addEventListener("DOMSubtreeModified", (e: any) => {
